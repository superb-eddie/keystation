...
const FIRMWARE_VERSION: &str = concat!("I am a keyboard :)", include!(concat!(
    env!("OUT_DIR"),
...
fn write_string(serial: &mut Serial, value: &str) {
    ...
    assert!(str_len < u8::MAX as usize, "String must be less than 255 characters");
...
fn main() -> ! {
    ...
    loop {
        keybed.scan(|key, state| match state {
            ...
            Key::Down(_) => send_note_down(serial, key as u8, 33),
            _ => {}
        });
    }
}
